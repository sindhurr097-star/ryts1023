...
import { calculateCostAnalysis } from '../services/claudeService';
import { AdvancedSignalProcessor } from '../services/signalProcessing';
...
export const useSensorSimulator = () => {
  ...
  useEffect(() => {
    const initialCostAnalysis = {};
    MACHINE_IDS.forEach(id => {
      initialCostAnalysis[id] = {
        loss: 0,
        maintenanceCost: 0,
        savings: 0,
    ...
    setCostAnalysis(initialCostAnalysis);
  }, []);
  ...
  // Initialize signal processors for each machine using useRef to persist across renders
  const signalProcessorsRef = useRef({});
  if (!signalProcessorsRef.current || Object.keys(signalProcessorsRef.current).length === 0) {
    signalProcessorsRef.current = {};
    MACHINE_IDS.forEach(machineId => {
      signalProcessorsRef.current[machineId] = {
        temperature_C: new AdvancedSignalProcessor(),
        vibration_mm_s: new AdvancedSignalProcessor(),
        rpm: new AdvancedSignalProcessor(),
        current_A: new AdvancedSignalProcessor()
      };
    });
  }
  const signalProcessors = signalProcessorsRef.current;
  ...
  const generateReading = useCallback((machineId) => {
    ...
    // Apply signal processing to filter noise (only if enough data has been collected)
    const tempProcessor = signalProcessors[machineId]?.temperature_C;
    const vibProcessor = signalProcessors[machineId]?.vibration_mm_s;
    const rpmProcessor = signalProcessors[machineId]?.rpm;
    const currentProcessor = signalProcessors[machineId]?.current_A;
    
    // Only apply filtering after 5 readings to allow processors to warm up
    const tempFiltered = tempProcessor && tempProcessor.history.raw.length >= 5 ? tempProcessor.process(temp).filtered : temp;
    const vibFiltered = vibProcessor && vibProcessor.history.raw.length >= 5 ? vibProcessor.process(vib).filtered : vib;
    const rpmFiltered = rpmProcessor && rpmProcessor.history.raw.length >= 5 ? rpmProcessor.process(rpm).filtered : rpm;
    const currentFiltered = currentProcessor && currentProcessor.history.raw.length >= 5 ? currentProcessor.process(current).filtered : current;
    
    // Update state with filtered values
    const newState = {
      ...state,
      temperature_C: tempFiltered,
      vibration_mm_s: vibFiltered,
      rpm: rpmFiltered,
      current_A: currentFiltered
    };
    ...
    const reading = {
      ...
      temperature_C: parseFloat(tempFiltered.toFixed(2)),
      vibration_mm_s: parseFloat(vibFiltered.toFixed(2)),
      rpm: Math.round(rpmFiltered),
      current_A: parseFloat(currentFiltered.toFixed(2))
    };
  ...
  const injectAnomaly = useCallback((machineId) => {
    ...
    // Temporarily disabled anomaly history tracking
    // setAnomalyHistory(prev => [{
    //   machineId,
    //   machineLabel: machines[machineId]?.label || machineId,
    //   sensor,
    //   timestamp: new Date().toISOString(),
    //   duration,
    //   peakValue: null
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      MACHINE_IDS.forEach(machineId => {
        ...
        // Calculate predicted failure hours based on risk score
        const predictedFailureHours = Math.max(1, Math.round((100 - riskScore) / 10));

        // Calculate cost analysis
        const analysis = calculateCostAnalysis(machineId, riskScore, predictedFailureHours);
        ...
        // Temporarily disabled anomaly history tracking
        // const currentAnomaly = anomalyState[machineId];
        // if (currentAnomaly?.active) {
        //   const sensorValue = reading[currentAnomaly.sensor];
        //   setAnomalyHistory(prev => prev.map(anomaly => {
        //     if (anomaly.machineId === machineId && anomaly.sensor === currentAnomaly.sensor && !anomaly.peakValue) {
        //       return { ...anomaly, peakValue: sensorValue };
        //     }
        //     return anomaly;
        //   }));
        // }
