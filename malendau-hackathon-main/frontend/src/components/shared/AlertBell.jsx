...
const AlertBell = ({ alerts, markAllRead, unreadCount }) => {
  ...
  const getSeverityColor = (severity) => {
    switch (severity) {
      ...
      default: return 'text-textMuted border-surfaceBorder';
    }
  };
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className="relative p-2 rounded-lg hover:bg-surfaceBorder transition-colors"
      >
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-96 bg-surface border border-surfaceBorder rounded-lg shadow-xl overflow-hidden z-50">
          {/* Header */}
          ...
          <div className="max-h-96 overflow-y-auto">
            {alerts.length === 0 ? (
              ...
              alerts.slice(0, 10).map((alert) => (
                <div
                  key={alert.id}
                  className={`p-4 border-l-4 ${getSeverityColor(alert.severity)} border-b border-surfaceBorder hover:bg-surfaceBorder/50 transition-colors`}
                >
          ...
          {/* Footer */}
          {alerts.length > 10 && (
            <div className="p-3 text-center text-sm text-textMuted border-t border-surfaceBorder">
