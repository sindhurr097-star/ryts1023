...
const Header = ({ alerts, markAllRead, toggleSidebar }) => {
  ...
  return (
    <header className="fixed top-0 right-0 left-0 lg:left-64 h-16 bg-surface border-b border-surfaceBorder z-30 flex items-center justify-between px-6">
      {/* Left side */}
      <div className="flex items-center gap-4">
        <h1 className="font-display font-bold text-primary text-xl flex items-center gap-2">
          <Zap size={24} />
          JnanikAI PredictMate
        </h1>
      </div>
